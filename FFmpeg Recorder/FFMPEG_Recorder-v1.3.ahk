...
RegExMatch(Incoming, "^http.*?(?=[|]|$)", Link0)
...
label1:
Progress, off
Run, %Recorder% %Uagent% %Referer% -i %Link% -c copy -f segment -segment_time 21600 -strftime 1 %PathFile% 
ExitApp
label2:
Progress, off
Run, %comspec% /c "%Recorder% %Uagent% %Referer% -i %Link% -c copy -f segment -segment_time 21600 -strftime 1 %PathFile% -c copy -f mpegts - | %Mplayer% -fs %Drawtext% -autoexit -"
ExitApp
